...
if users:
    ...
    for user in users:
        ...
        with st.expander(f"{user_full_name} ({user_email}) - {user_role}"):
            ...
            with col2:
                if user_id != current_user_id:  # Don't allow self-deactivation
                    if user_active:
                        confirm_deactivate = st.checkbox(
                            "Confirm deactivation",
                            key=f"confirm_deactivate_{user_id}"
                        )
            ...
            with col3:
                if user_id != current_user_id:  # Don't allow self role change
                    ...
                    confirm_role_change = st.checkbox(
                        f"Confirm change to {new_role}",
                        key=f"confirm_role_{user_id}"
                    )
